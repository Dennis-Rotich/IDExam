...
// modularized autosave functionality from the larger previous studentSubmission function
// to allow frequent invokation of autosave without using too much server memory
// 1. Autosave Controller (Fast, lightweight heartbeat)
const autosave = async (req, res) => {
    try {
        ...
        if (!result) {
            ...
            if (!result) {
                return res.status(404).json({ success: false, message: "Session not found or exam submitted." });
            }
        }
...
// 2. Student Submit Controller (Heavy code evaluation)
const studentSubmit = async (req, res) => {
    try {
        ...
        const submission = await submissionModel.findOne({ sessionId, status: "IN_PROGRESS" });
        if (!submission) {
        ...
        for (const testCase of problem.testCases) {
            const pistonPayload = {
                language: language, 
                version: "*",
                files: [{ name: "main", content: answer }], // 'answer' instead of 'code'
...
// 3. Run Code Controller (Stateless testing)
const runCode = async (req, res) => {
    try {
        ...
        const pistonPayload = {
            language: language, 
            version: "*",
            files: [{ name: "main", content: answer }],
            run_timeout: 3000,
            compile_timeout: 3000
        };
        ...
        const actualOutputClean = (compileCode !== 0) 
            ? compileOutput.trim() 
            : runOutput.trim();
